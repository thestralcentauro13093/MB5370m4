...
ggplot(data = diamonds) + 
  geom_bar(mapping = aes(x = cut, fill = clarity)) #BOH
...
ggplot(data = diamonds) + 
  geom_bar(mapping = aes(x = cut, fill = clarity), position = "fill") #make stacked bars th same height maintaining original proportions
...
```{r}
#Changing axis ticks frmo original graph
ggplot(mpg, aes(displ, hwy)) +
  geom_point() +
  scale_y_continuous(breaks = seq(15, 40, by = 5))
